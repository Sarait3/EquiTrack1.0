...
/**
 * Builds the HTML page that shows a list of equipment Includes search and
 * filter options, and actions based on user role
 */
public class ListViewBuilder extends PageBuilder {
	...
	/**
	 * Builds and returns the HTML page showing the equipment list
	 *
	...
	public String buildPage() {
		...
		// Search bar
		html.append("<div class='search-bar'>").append("<form class='search-form' method='GET' action='ListView'>")
		...
		html.append("<select name='statusFilter' class='search-input'>").append("<option value=''")
				...
				.append("<option value='operational'").append("operational".equals(statusFilter) ? " selected" : "")
				.append(">Operational</option>").append("<option value='out of service'")
				.append("out of service".equals(statusFilter) ? " selected" : "").append(">Out Of Service</option>")
				.append("</select>");
		...
		// Equipment items or empty message
		if (equipmentList.isEmpty()) {
			...
			for (Equipment eq : equipmentList) {
				...
				html.append("<a class='equipment-item' href='DetailView?id=").append(eq.getId()).append("'>")
						...
						.append(status.replaceAll(" ", "-")).append("'>").append(status.toUpperCase()).append("</span>")
						.append("</a>");
			}
		}
