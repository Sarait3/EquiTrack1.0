...
public class ManagerPageStrategy implements PageRoleStrategy {
	...
	/**
	 * Builds the sidebar for managers with links to all major pages
	 */
	@Override
	...
	/**
	 * Builds action buttons for equipment, based on operational status Managers can
	 * edit, check out, mark as returned, or delete equipment.
	 */
	@Override
	public String buildEquipmentActions(User user, Equipment equipment) {
		...
		html.append("<form method='GET' action='DetailView' style='display:inline-block;'>")
				...
				.append("</form>").append("</div></div>");

		return html.toString();
	}

	/**
	 * Builds approve/decline buttons for pending checkout requests
	 */
	@Override
	...
	/**
	 * Builds the HTML for account management (change password and email)
	 */
	@Override
	...
	/**
	 * Builds the list of non-admin users, with edit buttons for each.
	 */
	@Override
	public String buildUserList() {
		...
		for (User user : users.values()) {
			if (user.getRole().equalsIgnoreCase("regular")) {
				html.append("<tr>").append(String.format("<td>%s, %s</td>", user.getLName(), user.getFName()))
	...
	/**
	 * Managers do not use the create user feature, so this returns an empty string.
	 */
	@Override
